...
if "edit_id" in st.session_state:
    ...
    if q_to_edit:
        ...
        codes_text_default = json.dumps(q_to_edit['codes'], indent=4)
    ...
    codes_text_default = '{"1": "Male", "2": "Female"}'
    base_filter_default = ""
...
with st.form("question_form"):
    ...
    codes_text = st.text_area("Codes (JSON format)", value=codes_text_default)
    ...
    if submitted:
        try:
            ...
            codes = {int(k) if str(k).isdigit() else k: v for k, v in codes.items()}
        except:
            st.error("Codes must be valid JSON (e.g., {\"1\": \"Male\"})")
            st.stop()
